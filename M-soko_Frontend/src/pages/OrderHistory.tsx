...
const safeParseFloat = (value: number | string): number => {
  ...
  if (typeof value === 'string') {
    // Remove all characters except digits, a single decimal point, and optionally a leading minus sign
    const cleanedValue = value.replace(/[^\d.-]/g, '');
    return parseFloat(cleanedValue) || 0; // Return 0 if parsing still results in NaN
  }
...
const OrderHistoryPage = () => {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      const response = await axios.get('http://localhost:8000/api/orders/history/', { // Ensure this URL matches your backend
        headers: {
      ...
      setError('Failed to load order history. Please ensure you are logged in and try again later.');
    } finally {
      setLoading(false);
    }
                  ...
                  {order.items.map((item) => (
                    <ListItem key={item.id} disablePadding sx={{ py: 0.5 }}>
                      <ListItemText
                        primary={
                          <Typography variant="body1" sx={{ fontWeight: 'normal' }}>
                            {item.product.name} <Box component="span" sx={{ fontWeight: 'bold' }}>x{item.quantity}</Box>
                          </Typography>
                        }
                        secondary={
                          <Typography variant="body2" color="text.secondary">
                      ...
                      <Typography variant="body1" sx={{ fontWeight: 'bold' }}>
                        Kshs {(item.quantity * safeParseFloat(item.price)).toFixed(2)} {/* 👈 FIX APPLIED HERE */}
                      </Typography>
                    </ListItem>
                  ))}
                </List>
